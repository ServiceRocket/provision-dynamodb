...
const processConfigTable = async (table: ConfigTable) : Promise<any> => {
  ...
  const toWhatever = (target: 'READ'|'WRITE') => (accumulator, currentValue) => {
  ...
  const reads = readProvisioningChanges.reduce(toWhatever('READ'), {});
  const writesAndReads = writeProvisioningChanges.reduce(toWhatever('WRITE'), reads);

  Object.keys(writesAndReads).forEach(async tableOrGsiName => {
    const scalingInfo = writesAndReads[tableOrGsiName];
    if (scalingInfo.READ.isDefined || scalingInfo.WRITE.isDefined) {
      if (scalingInfo.READ.Type === 'Table' && scalingInfo.READ.Type === 'Table') {
        await updateTableCapacity(tableOrGsiName, scalingInfo.READ.apply, scalingInfo.WRITE.apply);
      } else if (scalingInfo.READ.Type === 'GSI' && scalingInfo.READ.Type === 'GSI') {
...
export const processConfigTables = async (tables : Array<ConfigTable>) : Promise<any> => await tables.forEach(processConfigTable);
